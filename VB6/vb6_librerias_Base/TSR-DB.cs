      ...
      public bool DBEjecutarSP(string psNombreSP, out int pnValRetorno, params object[] poParametros)
      {
         ...
         try
         {
               ...
               foreach (KeyValuePair<string, c_SP.stParametro> elem in dStoredProcedures[psNombreSP].Parametros)
	            {
                  ...
                  if (elem.Value.Tipo==SqlDbType.Char ||
                      elem.Value.Tipo==SqlDbType.NChar ||
                      elem.Value.Tipo==SqlDbType.VarChar ||
                      elem.Value.Tipo==SqlDbType.NVarChar)
                     loCom.Parameters[elem.Key].Size=8000;
               }
            }
            ...
            for (lnParam = 0; lnParam < poParametros.Length; lnParam += 2)
            {
               ...
               //ajusta el tamaño si es texto
               if (loCom.Parameters[poParametros[lnParam].ToString()].SqlDbType==SqlDbType.Char ||
                   loCom.Parameters[poParametros[lnParam].ToString()].SqlDbType==SqlDbType.NChar ||
                   loCom.Parameters[poParametros[lnParam].ToString()].SqlDbType==SqlDbType.VarChar ||
                   loCom.Parameters[poParametros[lnParam].ToString()].SqlDbType==SqlDbType.NVarChar ||
                   loCom.Parameters[poParametros[lnParam].ToString()].SqlDbType==SqlDbType.Text
                  )
                  loCom.Parameters[poParametros[lnParam].ToString()].Size=1000;
