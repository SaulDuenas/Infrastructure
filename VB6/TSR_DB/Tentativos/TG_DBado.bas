...
Option Explicit
''''********** TIPOS GLOBALES *****
'''Public Enum TTipoDB     'Tipo      Parámetros (ademas de User y Password)
...
'---------------------------------------------------------------------------
' CONSTANTE para el número de bases de datos
'---------------------------------------------------------------------------
...
Dim mUnattended As Boolean              'Indica si es una aplicación no atendida
...
' Parámetros:
'    pLogFileName$ - (IN):  Prefijo del archivo LOG, si es cadena vacía no genera Logs
'                           y envía los errores a pantalla.
'                           Se le concatena la fecha y se pone extensión .Log
'                           Ej. si el parámetro vale "MiApp" genera archivos
'                               MiAppYYYYMMDD.Log
'
...
'   pPwd$     (IN)     Sólo ODBC: Password,  para firmarse en la base de datos
...
'
 ...
 Dim lErrDB As ADODB.Error
   ...
   'Si hay conexión libre, se conecta
   If lOK Then
   ...
   Else
      ...
      FNErrorBox "No hay espacio para abrir otra conexión de base de datos"
   End If
...
'               Esta base de datos es la utilizada en el parámetro
