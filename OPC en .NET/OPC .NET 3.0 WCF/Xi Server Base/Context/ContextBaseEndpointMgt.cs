...
namespace Xi.Server.Base
{
		...
		where TList : ListRoot
	{
		...
		public virtual AliasResult OnAddListToEndpoint(string endpointId, uint serverListId)
		{
			...
			if (bValidEndpointId && bValidListId)
			{
				if (endpointEntry0.IsOpen)
				{
					endpointEntry1 = endpointEntry0 as EndpointEntry<ListRoot>;
					if (null != endpointEntry1 && null != tList)
					{
						return tList.OnAddListToEndpoint(endpointEntry1);
					}
				}
				return new AliasResult(XiFaultCodes.E_ENDPOINTERROR, tList.ClientId, tList.ServerId);
			}
			if (null != tList)
			{
				return new AliasResult(XiFaultCodes.E_BADENDPOINTID, tList.ClientId, tList.ServerId);
			}
			return new AliasResult(XiFaultCodes.E_BADLISTID, 0, serverListId);
		}
