...
namespace Xi.Server.Base
{
		...
		where TList : ListRoot
	{
		...
		{
			using (EnterMethod("IResourceManagement.Initiate", applicationName, workstationName, localeId))
			{
				try
				{
					...
					TraceSource.TraceEvent(TraceEventType.Verbose, 0, string.Format("Context being created for {0}", tContext.Identity.Name));
		...
		{
			// convert a "0" to the appropriate bits
			if (requestedContextOptions == 0)
			{
		...
		{
			using (EnterMethod("IResourceManagement.ReInitiate", existingContext))
			{
				try
				{
					...
					OperationContext ctx = OperationContext.Current;
					context.TransportSessionId = ctx.SessionId;
				}
				catch (FaultException<XiFault> fe)
				{
					throw fe;
		...
		void IResourceManagement.Conclude(
			string contextId)
		{
			using (EnterMethod("IResourceManagement.Conclude", contextId))
			{
				try
				{
		...
		void IResourceManagement.ClientKeepAlive(
			string contextId)
		{
			using (EnterMethod("IResourceManagement.ClientKeepAlive", contextId, DateTime.Now))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		ServerDescription IResourceManagement.Identify(
			string contextId)
		{
			using (EnterMethod("IResourceManagement.Identify"))
			{
		...
		List<ServerStatus> IResourceManagement.Status(
			string contextId)
		{
			using (EnterMethod("IResourceManagement.Status", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.LookupResultCodes", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.FindObjects", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.FindRootPaths", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		List<TypeAttributes> IResourceManagement.FindTypes(
			string contextId, FindCriteria findCriteria, uint numberToReturn)
		{
			using (EnterMethod("IResourceManagement.FindTypes", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		StandardMib IResourceManagement.GetStandardMib(
			string contextId)
		{
			using (EnterMethod("GetStandardMib", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("GetVendorMib", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.OpenEndpoint", contextId, contractType, endpointUrl))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.AddListToEndpoint", contextId, endpointId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.RemoveListsFromEndpoint", contextId, endpointId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.CloseEndpoint", contextId, endpointId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.DefineList", contextId, clientId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.GetListAttributes", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.RenewAliases", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.DeleteLists", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
					...
					return context.OnDeleteLists(listIds);
				}
				catch (FaultException<XiFault> fe)
				{
					throw fe;
		...
		{
			using (EnterMethod("IResourceManagement.AddDataObjectsToList", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.RemoveDataObjectsFromList", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.ModifyListAttributes", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.EnableListUpdating", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.EnableListElementUpdating", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.SetEventMessageFields", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
		...
		{
			using (EnterMethod("IResourceManagement.TouchDataObjects", contextId, listId))
			{
				try
				{
		...
		{
			using (EnterMethod("IResourceManagement.TouchList", contextId, listId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("ResourceManagement.GetAlarmSummary", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId);
