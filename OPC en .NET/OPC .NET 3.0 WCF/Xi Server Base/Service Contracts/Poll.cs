...
namespace Xi.Server.Base
{
		...
		where TList : ListRoot
	{
		...
		{
			using (EnterMethod("IPoll.PollDataChanges", contextId, listId))
			{
				try
				{
		...
		{
			using (EnterMethod("IPoll.PollEventChanges", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("IPoll.PollPassthroughResponses", contextId))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
