...
namespace Xi.Server.Base
{
									...
									where TList : ListRoot
	{
		...
		{
			using (EnterMethod("IWrite.WriteValues", contextId, listId, writeValueArrays))
			{
				try
				{
		...
		{
			using (EnterMethod("IWrite.WriteVST", contextId, listId, writeValueArrays))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("IWrite.WriteJournalData", contextId, listId, modificationType, valuesToWrite))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("IWrite.WriteJournalEvents", contextId, listId, modificationType, eventsToWrite))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("IWrite.AcknowledgeAlarms", contextId, listId, operatorName, comment, alarmsToAck))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
		...
		{
			using (EnterMethod("IWrite.Passthrough", contextId, recipientId, passthroughName, DataToSend))
			{
				try
				{
					TContext context = ContextManager<TContext, TList>.LookupContext(contextId, false);
