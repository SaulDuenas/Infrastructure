...
2. Static buffer overrun on the stack. (See: StaticBufferOverrun)
...
1. If the overwritten return address points to an invalid memory, the 
application crashes with Access Violation once the current function returns. 
(See: ArrayIndexingError/b)

2. If the overwritten return address happens to point to a valid memory, 
...
3. If the overwritten return address points to malicious codes that attackers 
injected, it will cause serious security problems.
...
Advanced Windows Debugging: Chapter 5. Memory Corruptions Part I: Stacks
http://advancedwindowsdebugging.com/index.html
