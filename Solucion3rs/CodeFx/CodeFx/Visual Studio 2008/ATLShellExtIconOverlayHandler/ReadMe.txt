...
overlay handler for files with "CodeFx" in the file paths. After installing 
the icon overlay handler, any file with "CodeFx" in its file path, e.g. 
"D:\CodeFx.txt", shows an icon overlay in Windows Explorer. 
				...
				NoRemove Windows
				{
					NoRemove CurrentVersion
					{
						NoRemove Explorer
						{
							...
							{
								ForceRemove CodeFxATLShellExtIconOverlayHandler = 
...
were specified by the handlers' GetPriority method during initialization.
	...
	IFACEMETHODIMP CFileIconOverlayExt::IsMemberOf(LPCWSTR pwszPath, 
												   DWORD dwAttrib)
	{
		// If the file name contains "CodeFx", add the overlay
		if (wcsstr(pwszPath, L"CodeFx") != 0)
			return S_OK;
		else
			return S_FALSE;
	}
