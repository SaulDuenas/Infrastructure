...
1. Implicitly Define the Class Interface with 
...
generally recommended because it's creedless to COM versioning.

2. Explicitly Define the Class Interface with
[ClassInterface(ClassInterfaceType.None)]
and the implementation of real interfaces that defines the properties, 
methods and events to be exposed.

In contrast with the first method, defining an interface explicitly, deriving 
your .NET Component class from the interface and then implementing the 
interface's methods in your .NET Component is the recommended way of doing 
things if you are going to expose your .NET class to COM aware clients.
...
the project references the Microsoft.VisualBasic assembly.

CSDllCOMServer exposes the following items:

1. CSSimpleObject1 whose class interface is implicitly defined.
...
2. CSSimpleObject2 whose class interface is explicitly defined.
...
B. Adding a component whose interface is implicitly defined

Step1. Define a "public" class CSSimpleObject1 and attach the attribute 
[ClassInterface(ClassInterfaceType.AutoDual)] or 
[ClassInterface(ClassInterfaceType.AutoDispatch)] to it.
ClassInterface(ClassInterfaceType.AutoDual/AutoDispatch) exports the public 
members of the .NET Component's class into a default (implicit) Class 
Interface in the generated type-library.

Step2. Make sure that the constructor of the class CSSimpleObject1 is not 
...
Step3. Add a public property, float FloatProperty, and two public methods, 
HelloWorld & GetProcessThreadID, to the class.

Step4. Build the project. The GUIDs (i.e. CLSID and IID) of CSSimpleObject1
are generated during the compile time.

C. Adding a component whose interface is explicitly defined

Step1. Define a "public" interface ICSSimpleObject2 to describe the COM 
interface of the coclass. Specify its GUID, aka IID, using GuidAttribute: 
...
Step2. Inside the interface ICSSimpleObject2, define the prototypes of the 
properties and methods to be exported. The terms marked as 
[ComVisible(false)] are not exported.

Step3. Define a "public" interface ICSSimpleObject2Events to describe the 
events the coclass can sink. Specify its GUID, aka the Events Id, using 
GuidAttribute:
...
Step4. Inside the interface ICSSimpleObject2Events, define the prototype of
the events to be exported.

Step5. Define a "public" class CSSimpleObject2 that implements the interface
ICSSimpleObject2. Attach the attribute 
...
the ICSSimpleObject2 interface is the default interface. In addition, specify 
...
Step6. Make sure that the constructor of the class CSSimpleObject2 is not 
private (we can either add a public constructor or use the default one), so 
that the COM object is creatable from the COM aware clients.

Step7. Inside CSSimpleObject2, implement the interface ICSSimpleObject2 by 
writing the body of the property FloatProperty and the methods HelloWorld,  
GetProcessThreadID and HiddenFunction.
