...
* 
* 	1. CodeFx C1 - display the file path when enabled
* 	2. CodeFx C2 - display the file size when enabled
...
* 
* History:
* * 9/19/2009 8:30 PM Jialiang Ge Created
\***************************************************************************/
...
IFACEMETHODIMP CFileColumnExt::GetColumnInfo(DWORD dwIndex, SHCOLUMNINFO* psci)
{
	...
	case 0:		// Column 1: CodeFx C1
		...
		wcsncpy_s(psci->wszTitle, MAX_COLUMN_NAME_LEN, 
			L"CodeFx C1", MAX_COLUMN_NAME_LEN);
		wcsncpy_s(psci->wszDescription, MAX_COLUMN_DESC_LEN,
			L"CodeFx Column 1", MAX_COLUMN_DESC_LEN);
		break;

	case 1:		// Column 2: CodeFx C2
		psci->scid.fmtid = CLSID_FileColumnExt;	// Use our CLSID as format ID
		...
		// Set column title and description
		wcsncpy_s(psci->wszTitle, MAX_COLUMN_NAME_LEN, 
			L"CodeFx C2", MAX_COLUMN_NAME_LEN);
		wcsncpy_s(psci->wszDescription, MAX_COLUMN_DESC_LEN, 
			L"CodeFx Column 2", MAX_COLUMN_DESC_LEN);
		break;
