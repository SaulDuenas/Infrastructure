...
This CSASPNETFormViewUpload sample describes how to display and upload 
...
Default populates a FromView control with data from a SQL Server database 
...
After that, fill the generated methods with the sample code.
    ...
    {
        cmd.Parameters.Add("@Picture", SqlDbType.Image).Value = uploadPicture.FileBytes;
    }
    else
    {
...
bytes array, specify the and then write the array to the HTTP output stream 
