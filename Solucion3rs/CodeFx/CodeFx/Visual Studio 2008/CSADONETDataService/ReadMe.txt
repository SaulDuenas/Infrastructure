...
D. Creating an ADO.NET Data Service for non-relation CLR objects.  
   ...
   1. Add a new ADO.NET Data Service named CodeFx.svc.

   2. Creating the in-memory data classes.

      1) Create class Category with property CategoryName to represent the 
         CodeFx project category, and set the data service key value as 
         CategoryName.

      2) Create class Project with properties ProjectName, Owner, and 
         ProjectCategory to represent the CodeFx project, and set the data 
         service key value as ProjectName.

   3. Creating the ADO.NET Data Service entity class for non-relational 
      CLR objects.

      1) Create a class named CodeFxProjects and set the CodeFx.svc Data 
         Service targeting to the CodeFxProjects class.
 
      2) Declare two static members for CodeFxProjects class, categories 
         in type of List<Category> and projects in type of List<Project>, 
         and a static constructor for CodeFxProjects class to initialize 
    ...
    4. Implementing the IUpdatable methods to enable the insert 
       functionality of the ADO.NET Data Service entity class CodeFxProjects.
