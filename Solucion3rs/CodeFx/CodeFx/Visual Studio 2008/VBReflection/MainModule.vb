...
' 
' History:
' * 4/6/2009 11:24 PM Jialiang Ge Created
'***************************************************************************'
...
Module MainModule
    ...
    Sub Main()
        ...
        Dim type As Type = assembly.GetType("VBClassLibrary.VBSimpleClass")
        Dim obj As Object = Activator.CreateInstance(type, New Object() {0.0F})
        ...
        Dim publicMethod As MethodInfo = type.GetMethod("Increment")
        ' Examine the method parameters
        Dim Params As ParameterInfo() = publicMethod.GetParameters()
        For Each Param As ParameterInfo In Params
            Console.WriteLine("Param={0}", Param.Name)
            Console.WriteLine(" Type={0}", Param.ParameterType)
            Console.WriteLine(" Position={0}", Param.Position)
        Next
        Dim result As Object = publicMethod.Invoke(obj, New Object() {1.2F})
        Console.WriteLine("Result={0}" & vbCrLf, result)

        ' Call a Internal Function

        Console.WriteLine("Call the friend (internal) method, InternalIncrement")
        result = type.InvokeMember("InternalIncrement", _
            BindingFlags.InvokeMethod Or _
            BindingFlags.Instance Or BindingFlags.NonPublic, _
            Nothing, obj, New Object() {2.1F})
        Console.WriteLine("Result={0}" & vbCrLf, result)

        ' Call a Static Function

        Console.WriteLine("Call the shared (static) method, StaticMethod")
        result = type.InvokeMember("StaticMethod", _
            BindingFlags.InvokeMethod Or _
